import airpods1 from "./../assets/images/airpods.svg";

import hPh1 from "./../assets/images/hph1.svg";
import hPh2 from "./../assets/images/hph2.svg";

import cases1 from "./../assets/images/cases1.svg";
import cases2 from "./../assets/images/cases2.svg";
import cases3 from "./../assets/images/cases3.svg";
import case1 from "./../assets/images/case1.svg";
import case2 from "./../assets/images/case2.svg";
import case3 from "./../assets/images/case3.svg";
import case4 from "./../assets/images/case4.svg";
import case5 from "./../assets/images/case5.svg";
export const dataBase = [
  {
    headphones: [
      {
        id: 1,
        img: hPh1,
        name: "Apple BYZ S852I",
        oldPrice: 3527,
        newPrice: 2927,
        headphonesImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
      {
        id: 2,
        img: hPh1,
        name: "Apple BYZ S852I",
        oldPrice: 3527,
        newPrice: 2937,
        headphonesImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
      {
        id: 3,
        img: hPh2,
        name: "Apple BYZ S852I",
        oldPrice: 3527,
        newPrice: 2527,
        headphonesImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
      {
        id: 4,
        img: hPh1,
        title: "Apple BYZ S852I",
        oldPrice: 3527,
        newPrice: 2927,
        headphonesImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
      {
        id: 5,
        img: hPh2,
        title: "Apple BYZ S852I",
        oldPrice: 3527,
        newPrice: 2927,
        headphonesImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
      {
        id: 6,
        img: hPh1,
        title: "Apple BYZ S852I",
        oldPrice: 3527,
        newPrice: 2927,
        headphonesImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
    ],
  },
  {
    cases: [
      {
        id: 7,
        img: cases1,
        description: "Стеклянные",
        price: 3000,
        casesImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
      {
        id: 8,
        img: cases2,
        description: "Силиконовые",
        price: 3000,
        casesImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Активное шумоподавление: Нет" },
          { id: 2, description: "Вес: 15 гр" },
          { id: 3, description: "Материал: Силиконовые" },
          { id: 5, description: "Цвет: Желтый" },
          { id: 6, description: "Артикул: 1223326" },
        ],
      },
      {
        id: 9,
        img: cases3,
        description: "Кожаные",
        price: 3000,
        casesImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Активное шумоподавление: Нет" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Влагозащита: Нет" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
    ],
  },
  {
    airpods: [
      {
        id: 10,
        img: airpods1,
        name: "Apple BYZ S852I",
        newPrice: 2927,
        oldPrice: 3527,
        rating: 4.7,
        airpodsImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
      {
        id: 11,
        img: airpods1,
        name: "Apple FRd S852I",
        newPrice: 2500,
        oldPrice: 3527,
        rating: 4.7,
        airpodsImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
      {
        id: 12,
        img: airpods1,
        name: "Apple BYZ S852I",
        newPrice: 3000,
        oldPrice: 3527,
        rating: 4.7,
        airpodsImg: [
          {
            id: 1,
            img: case1,
            text: "case",
          },
          {
            id: 2,
            img: case2,
            text: "case",
          },
          {
            id: 3,
            img: case3,
            text: "case",
          },
          {
            id: 4,
            img: case4,
            text: "case",
          },
          {
            id: 5,
            img: case5,
            text: "case",
          },
        ],
        descriptionText: [
          { id: 1, description: "Ударостойкость: 4 из 10" },
          { id: 2, description: "Вес: 10 гр" },
          { id: 3, description: "Материал: Стеклянные" },
          { id: 5, description: "Цвет: Белый" },
          { id: 6, description: "Артикул: 122ек26" },
        ],
      },
    ],
  },
];
